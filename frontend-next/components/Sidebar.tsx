...
export function Sidebar({ docs, onUploaded, onDelete, onToast }: Props) {
  ...
  async function upload(files: File[]) {
    ...
    try {
      ...
      if (data.failed > 0) {
        const failure = data.results?.find((result: { status: string }) => result.status === 'error');
        const reason = failure?.error || failure?.message || 'Unknown indexing error';
        onToast(`${data.successful} indexed · ${data.failed} failed: ${reason}`);
      } else {
        onToast(`${data.successful} file(s) ready`);
      }
      onUploaded();
    } catch (e: unknown) {
